...
def generateData(batch_size, data=[]):
    ...
    while True:
        ...
        for i in (range(len(data))):
            ...
            label = load_img(filepath + 'NewLB/' + url, grayscale=True)
    ...
    while True:
        ...
        for i in (range(len(data))):
            ...
            if batch % batch_size == 0:
                ...
                # print("valid_data.shape,valid_label.shape = ",valid_data.shape,valid_label.shape)
...
def train():
    ...
    # model = seg_fc_hrnet()
    from qt_model.Model.my_unet import Unet
    ...
    # 保存H
    with open('训练记录/TrainHistory-Seg_resnet50_unet-1_label-50_epoch_again.data', 'wb') as file_pi:
        pickle.dump(H.history, file_pi)

    # 保存模型方法1
    model_save_path = r"训练权重/seg_resnet50_unet-model_50_only_1_again.h5"
    ...
    plt.show()
    plt.savefig("result_resnet50_unet_again.png")
